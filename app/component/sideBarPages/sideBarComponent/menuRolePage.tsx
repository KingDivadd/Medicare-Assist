                    ...
                    {[1,2,3].map((data, ind)=>{
                        ...
                        return (
                            <span onClick={()=> {setSelecteMenuRole({ind, data}); setMenuRoleModal(true)}} key={ind} className="w-full flex flex-row items-center justify-start h-[35px] hover:bg-slate-100 cursor-pointer ">
                                <p className="text-sm h-full flex items-center justif-start  text-blue-500 w-[250px] px-2">{'email'.toUpperCase()}</p>
                                <p className="text-sm h-full flex items-center justif-start  text-slate-700 w-[400px] px-2">{'first_name'.toUpperCase()}</p>
                            </span>  
                        )
                    })}                 
                </div>
