    ...
    return (
        <nav className='w-full bg-sky-700 flex flex-row items-center justify-between positon-fixed'>
            <div className="flex flex-row gap-1 items-center w-auto h-[50px]">
                {['admin-1', 'admin-2','billing', 'front-desk', 'medical-assistant', 'office-manager', 'physicians'].includes(userRole) &&  <Link href={'/home/dashboard'} className={page === '/home/dashboard'? 'active-route-nav-item':'route-nav-item'}>
                    <BiHomeAlt2 size={23} className="text-slate-300" />
                    <p className="text-slate-300 text-[15px]">Home</p>
                </Link>}
                
                {['admin-1', 'admin-2', 'medical-assistant', 'office-manager', 'physicians'].includes(userRole) &&  <Link href={'/'} className={page === '/'? 'active-route-nav-item':'route-nav-item'}>
                    <BiNotepad size={23} className="text-slate-300" />
                    <p className="text-slate-300 text-[15px]">EHR</p>
                </Link>}
                
                {['admin-1', 'admin-2','billing', 'front-desk', 'office-manager',].includes(userRole) &&  <Link href={'/patient'} className={page === '/patient'? 'active-route-nav-item':'route-nav-item'}>
                    <GoPerson size={23} className="text-slate-300" />
                    <p className="text-slate-300 text-[15px]">Patient</p>
                </Link>}
                
                {['admin-1', 'admin-2','billing', 'front-desk', 'medical-assistant', 'office-manager', 'physicians'].includes(userRole) &&  <Link href={'/scheduling'} className={page === '/scheduling'? 'active-route-nav-item':'route-nav-item'}>
                ...
                {['admin-1', 'admin-2','billing', 'front-desk', 'medical-assistant', 'office-manager', 'physicians'].includes(userRole) &&  <Link href={'/task-management'} className={page === 'task-management'? 'active-route-nav-item':'route-nav-item'}>
            ...
            <div className="flex flex-row gap-2 items-center h-[70%] items-center pr-5">
                {['admin-1', 'admin-2','billing', 'front-desk', 'medical-assistant', 'office-manager', 'physicians'].includes(userRole) &&  <Link href={'/new-patient'} className='flex flex-row gap-2 h-full items-center justify-center px-3 bg-amber-500 cursor-pointer rounded-[3px]'>
